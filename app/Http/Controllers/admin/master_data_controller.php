...
class master_data_controller extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pro_3m.admin.update-master.master-index');
    }
    public function table_show($table)
    {
        return view('pro_3m.admin.data-table.table-view', compact('table'));
    }
    ...
    public function add_check_list_name(Request $request)
    {
        $table = 'App\Models\\' . $request->input('table');
        if ($request->ajax()) {
            if (class_exists($table)) {
                $data = $table::all();
                $colum = array_keys($data->first()->getAttributes());
                $colums = array_diff($colum, ['created_at', 'updated_at', 'id']);
                $data_request = $request->only($colums);

                $record = $table::where($data_request)->first();
                if ($record) {
                    return response()->json([
                        'status' => 400,
                    ]);
                } else {
                    $table::create($data_request);
                    return response()->json([
                        'status' => 200,
                    ]);
                }
